...
**ZIP code vs county FIPS:** a bare 5-digit number is ambiguous — 10001 is both a Manhattan ZIP code and the county FIPS of Kent County, DE. In deep links, EJScreen follows EJAM's convention: `?wherestr=10001` is tried as a **county FIPS first** (drawing and selecting the county boundary), and only geocoded the old way (which reads 5 digits as a ZIP) if no county has that code. To be unambiguous, pass FIPS codes with `fips=`, and force a ZIP by adding context (e.g. `?wherestr=10001,NY`). The interactive search box inside the app is unchanged and still treats a bare 5-digit number as a ZIP code.
